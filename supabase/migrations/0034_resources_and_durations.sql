...
-- And record_call has to carry it -------------------------------------------
-- The appointment it writes was the only one with no length: everything else
-- goes in through the webhook, which sets its own. Without this, the first
-- booking of every call falls back to sixty minutes and a twenty minute
-- check-up blocks two thirds of an hour that nobody is in.
...
begin
  ...
  if p_appointment is not null and p_appointment <> 'null'::jsonb then
    ...
    insert into appointments (clinic_id, call_id, patient_name, patient_phone, reason, scheduled_at, origin, summary, duration_minutes)
    values (
